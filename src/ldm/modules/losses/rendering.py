...
def rand_range(shape, low, high, dtype=torch.float32): # Sy: shape = (1, 9, 1)
...
def generate_normalized_random_direction(
    batchSize, nbRenderings, lowEps=0.001, highEps=0.05 # Sy: batchSize=1, nbRenderings=9
):
    r1 = rand_range(
        (batchSize, nbRenderings, 1), 0.0 + lowEps, 1.0 - highEps, dtype=torch.float32
    ) # Sy: Generate a random float number 0. ~ 1.
    r2 = torch.rand([batchSize, nbRenderings, 1], dtype=torch.float32) # Sy: torch.rand => [0,1) Uniform random numbers.
    r = torch.sqrt(r1)
    phi = 2 * math.pi * r2 # Sy: phi has random value withn [0, 2pi). The purpose is to generate random wi or wo. The purpose is to do Motecarlo Integration.
    # min alpha = atan(sqrt(1-r^2)/r)
    x = r * torch.cos(phi) # Sy: In order to define a view or light position. Define x&y on the horiziontal plane. 
    y = r * torch.sin(phi)
    z = torch.sqrt(1.0 - torch.square(r)) # Sy: We define the height of the light position witin [0,1]. z^2 = 1 - r^2 = 1 - r1.
    finalVec = torch.cat(
        [x, y, z], axis=-1
...
class GGXRenderer:
    ...
    # def compute_diffuse(self, basecolor, metallic): # Sy : we use basecolor and metallic here
    #     diffuse = basecolor * (1.0 - metallic)
    #     specular = basecolor * metallic + 0.04
    #     return diffuse * (1.0 - specular) / math.pi
    ...
    # def compute_fresnel(self, basecolor, metallic, VdotH): # Sy:
    #     specular = basecolor * metallic + 0.04
    #     sphg = torch.pow(2.0, ((-5.55473 * VdotH) - 6.98316) * VdotH)
    #     return specular + (1.0 - specular) * sphg
    ...
    def calculateBRDF(
        ...
        # Sy: basecolor, metallic -> diffuse, specular
        diffuse = basecolor * (1.0 - metallic)
        specular = basecolor * metallic + 0.04

        diffuse_rendered = self.compute_diffuse(diffuse, specular)
        # diffuse_rendered = self.compute_diffuse(basecolor, metallic)
        ...
        # F_rendered = self.compute_fresnel(basecolor, metallic, torch.clamp(VdotH, min=0.0))

        specular_rendered = F_rendered * (G_rendered * D_rendered * 0.25) # Sy: FGD/4
    ...
    # def generateDiffuseRendering(self, batchSize, nbRenderings, targets, outputs): # Sy: (called from materialloss) batchSize = 1, nbRanderings = 9(=diffuse(3)+specular(3)+normal(2)+roughness(1)), targets = inputs, outputs = reconstructions
    def generateRendering(self, batchSize, nbRenderings, targets, outputs):    
        currentViewPos = generate_normalized_random_direction(
            batchSize, nbRenderings, lowEps=0.001, highEps=0.1
        ) # Sy: currentVeiwPos = [1, 9, 3]. Cam pose
        currentLightPos = generate_normalized_random_direction(
            batchSize, nbRenderings, lowEps=0.001, highEps=0.1
        ) # Sy: currentLightPos = [1, 9, 3]. It means we use point light.

        wi = currentLightPos # Sy: Random light position
        wi = torch.unsqueeze(wi, axis=2)
        wi = torch.unsqueeze(wi, axis=2) # Sy: [1, 9, 1, 1, 3]

        wo = currentViewPos # Sy: Random veiw position
        ...
        renderedDiffuse = self.render( # Sy: wi & wo are randomly sampled direction.
            targets, wi, wo, None, "diffuse", useAugmentation=False, lossRendering=True
        )[0] # Sy: rendering the inputs([b, 256, 256, 12])

        renderedDiffuseOutputs = self.render(# Sy: rendering the reconstructions
        ...
        return [renderedDiffuse, renderedDiffuseOutputs] # Sy: Rather than using an already rendered image, they put x and xrec into the Render eq and counted the difference.
    ...
    def generateSpecularRendering(
        ...
        renderedSpecular = self.render( # Sy: There is no place in self.render that takes "specu" as a param.
