...
class FrozenCLIPSentenceEmbedder(AbstractEncoder):
    ...
    def encode(self, text, **kwargs):
        return self.transformer.encode(text, **kwargs) # Sy: 여기서 torch.Size 쭈루룩
...
class FrozenCLIPImageEmbedder(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.model.encode_image(self.preprocess(x)).type(torch.float32).unsqueeze(1) # Sy: 여기서 torch.Size 쭈루룩
        if self.normalize:
