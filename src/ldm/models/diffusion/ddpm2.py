...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        if self.ucg_training:
            ...
            ucg_all = self.ucg_training.pop("all")
    ...
    def get_input(self, batch, k):
        x = batch[k] # Sy: batch is dict and the keys of batch is {'image_embed', 'maps', 'packed'}. k is 'Packed'. -> It should be changed to 'packed'
        if k == "text": # Sy: k가 'text'면 batch[k]부터 에러가 나서 위와 같이 수정함.
            return x
        # # Sy: AttributeError: 'dict' object has no attribute 'shape' 해결을 위한 수정 -> ??
        # if isinstance(x, dict):
        #     maps = x.keys()
        #     for map in maps:
        #         if len(x[map].shape) == 3: # Sy: 제대로 된 map이 들어오면 len(x[map].shape) == 4 이다. [b, c, h, w]
        #             x = x[map][..., None] # Sy: batch dim을 추가하기 위한 것 같은데 각 map에 batch 차원을 맘대로 추가해도 되는건가?
        
        # else:
        #     if len(x.shape) == 3: # Sy: batch['maps'] = {'diffuse', 'nor', 'rough', 'spec'} 이런 map으로 또 나뉘기에 AttributeError: 'dict' object has no attribute 'shape' 발생.
        #         x = x[..., None] # Sy: tensor의 마지막 차원 1개 추가
        if len(x.shape) == 3: 
            x = x[..., None]
        x = rearrange(x, "b h w c -> b c h w") # Sy: k = maps일 경우 문제가 발생...
    ...
    def training_step(self, batch, batch_idx):
        keep_cond = self.ucg_prng.choice(
            ["none", "all", "mixed"], p=[self.p_drop, self.p_keep, self.p_mixed] # Sy: 0.1 / 0.1 / self.p_mixed = 0.8
        )

        for k in self.ucg_training: # Sy: self.ucg_training = {'render': {'p': 0.5, 'val': 0.0}(same), 'image_embed': (same), 'palette': (same), 'sketch': (same), 'text': (same)} -> remove keys('render, palette, sketch')
            p = self.ucg_training[k]["p"]
            val = self.ucg_training[k]["val"]
            if val is None:
                val = ""
            for i in range(len(batch[k])): # Sy: self.ucg_training = ucg_training. ucg_training is in yaml.
                if keep_cond == "none":
                    batch[k][i] = val
                elif keep_cond == "all":
                    pass
        ...
        if (batch_idx + 1) % 200 == 0:
            loss_dict.update({"epoch": self.current_epoch})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx % 100 == 0:
            loss_dict_ema.update({"epoch": self.current_epoch})
            loss_dict_no_ema.update({"epoch": self.current_epoch})
            # wandb.init(project="matGen", entity="sogang_swatchon") # Sy:
            # wandb.log(loss_dict_ema)
            # wandb.log(loss_dict_no_ema)
            # self.log_images(batch, N=4, inpaint=False, plot_progressive_rows=False)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        diffusion_ckpt = kwargs.pop("diffusion_ckpt", [])
        ...
        if diffusion_ckpt:
            self.init_from_ckpt(diffusion_ckpt)
    ...
    def get_first_stage_encoding(self, encoder_posterior): # Sy: Set Random Noise and Encoded Input to z.
        if isinstance(encoder_posterior, DiagonalGaussianDistribution): # Sy: This is random noise part
            z = encoder_posterior.sample()
        elif isinstance(encoder_posterior, torch.Tensor): # Sy: encoder_posterior.shape = [b, 12, 32, 32] = b, c(d,n,r,s), h, w
            z = encoder_posterior
        else:
            raise NotImplementedError(
    ...
    def get_learned_conditioning(self, c): # Sy: c = {'packed': [b, 12, 256, 256], 'image_embed': [b, 3, 256, 256], 'text': [' ']}
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, "encode") and callable(
                self.cond_stage_model.encode # Sy: self.cond_stage_model is MultiConditionEncoder.forward() . It is in "src/ldm/modules/encoders/multicondition.py".
            ):
    ...
    def get_input(
        ...
        x = super().get_input(batch, k) # Sy: k is 'Packed' -> 'packed'
        if bs is not None:
        ...
        if self.model.conditioning_key is not None: # Sy: self.model.conditioning_key is 'hybrid'
            if cond_key is None: 
                cond_key = self.cond_stage_key # Sy: self.cond_stage_key is ['render', 'image_embed', 'sketch', 'palette', 'text'] -> Use this as k for get_input
            if cond_key != self.first_stage_key: # Sy: self.first_stage_key = 'packed'
                if cond_key in ["caption", "coordinates_bbox", "txt"]:
                    xc = batch[cond_key]
                elif cond_key == "class_label":
                    xc = batch
                elif isinstance(cond_key, ListConfig):
                    # NOTE I chose to leave the plain text here and to emebd it in the multicondition module.
                    # I find it more intuitive, but this requires additional conditions here as get_input()
                    # operates on tensors.
                    xc = { 
                        key: super(self.__class__, self) # Sy: <super: <class 'LatentDiffusion'>, <LatentDiffusion object>>
                        .get_input(batch, key) # Sy: key is ['render', 'image_embed', 'sketch', 'palette'], and get_input(batch, "render") returns Key Error
                        .to(self.device)
                        for key in cond_key
                        if key != "text"
                    }
        ...
        if return_original_cond:
            out.append(xc) # Sy: xc is input dict {'packed': [b, 12, 256, 256], 'image_embed': [b, 3, 256, 256]}
        return out
...
#         )
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if (
                ...
                # print(patch_limits_tknzd[0].shape) # Sy:
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, image_size=None, **kwargs):
        ...
        return samples, intermediates # Sy: samples = [b, 12, 32, 32], intermediates['x_inter'][0].shape = [b, 12, 32, 32]
    ...
    def get_unconditional_conditioning(self, batch_size, null_label=None):
        ...
        if isinstance(c, dict):
            c = {
                key: repeat(c[key], "1 ... -> b ...", b=batch_size).to(self.device)
                for key in c
            }
        else:
    ...
    def log_images(
        ...
        **kwargs,
        ...
        for k in self.ucg_training.keys() - cond:
            val = self.ucg_training[k]["val"]
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["image_embed"] = xc["image_embed"]
                # log["sketch"] = xc["sketch"]  # Sy: remove the sketch & palette logs.
                # palettes = (xc["palette"] * 255).type(torch.uint8)
                # palettes = palettes.squeeze(1)
        ...
        if sample:
            ...
            with ema_scope("Sampling"):
                samples, z_denoise_row = self.sample_log(
                    cond=c, # Sy: c = { 'c_crossattn': [8, 2, 512] }
                    batch_size=N,
                    ddim=use_ddim,
        ...
        if inpaint:
            ...
            with ema_scope("Plotting Inpaint"):
                # TODO Add for to log all condition combinations
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t) # Sy: diffusion_model refers to UNet. x refers to latent tensor being denoised.
        elif self.conditioning_key == "concat":
            xc = torch.cat([x] + c_concat, dim=1) # Sy: c_concat is refers to the sketch image.
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == "crossattn":
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc) # Sy: context refers to cross attention condition
        elif self.conditioning_key == "hybrid": # Sy: In the case of matfuse's conditioning_key is hybrid.
            # Sy: c_concat is None. -> It should be a tensor.
            if c_concat is None:
                xc = x # Sy: c_concat is None. -> c_concat should be a tensor and shape of dim=1 should be 8? (X) => Change yaml file unet_config: in_channels: 20 -> 12
                # batch = x.shape[0]
                # xc = torch.cat([x, torch.zeros([batch, 8, 32, 32]).to(self.device)], dim=1)
            else:
