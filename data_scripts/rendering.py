...
class Rendering():
    ...
    def __init__(self, data_root, device=torch.device("cuda:0"), size=256, input_pbr_names=["basecolor", "normal", "roughness", "metallic"]): # Sy: Why just diffuse only? (torch.device("cuda:0") or torch.device("cuda:1") or torch.device("cuda:2"))
        ...
        self.materials = [
            ...
            for x in self.data_root.glob("**/**/basecolor.png") # Sy: for example x is '/sample_materials/asphalt_floor/diffuse.png'
        ]
        ...
        # Sy:
        self.envs = [
            {"name": y.stem, "folder": y.parent}
            for y in self.data_root.glob("envmap_*/*_0.png")
        ]
        self.env_lenght = len(self.envs)
    ...
    def __len__(self):
        return self.pbr_length, self.env_lenght
    ...
    def render_env(self, folder, pbrmap, envmap, env_name, device):
        ...
        save_dir = folder/("render_" + env_name)
    ...
    def render(self, number): # Sy: number = rendering 할 env map 갯수
        for i in tqdm(range(self.pbr_length)):
            ...
            check_renders = glob.glob(str(folder/"render_*"))
            if (len(check_renders) == number):
                continue
            # # elif len(check_renders) > 10:
            # #     print(f"more than 10 render images in {folder}")
            else:
                for render_img_path in check_renders:
                    directory_to_delete = Path(render_img_path)
                    print(f"Deleting directory and all its contents: {directory_to_delete}")
            ...
            env_list = random.sample(self.envs, number) # Sy: Sample env map randomly. len(self.envs) is about more than 1,000.
            
            if len(env_list) % num_devices!=0: # Sy: device 갯수의 배수만큼
                raise ValueError(f"The input list length must be a multiple of {num_devices}. Make the value of 'number' a multiple of 'num_devices'.")
            env_chunks = [env_list[i:i+num_devices] for i in range(0, len(env_list), num_devices)]
            
            # processes = []
            # for env_chunk in env_chunks:
            #     for device_idx, env in enumerate(env_chunk):
            #         env_folder = env["folder"]
            #         env_name = env["name"]
            #         env_src = env_folder / (env_name + ".png")
            #         envmap_img = self.render_preprocess(env_src, env_name)
            #         p = mp.Process(target=self.render_env, args=(folder, pbrmap, envmap_img, env_name, devices[device_idx]))
            #         p.start()
            #         processes.append(p)
            #         print(p)
            ...
            for env_chunk in env_chunks:
                env_folder1 = env_chunk[0]["folder"] # Sy: env_chunk[0] is the first env_map dict in env_chunk.
                env_name1 = env_chunk[0]["name"]
                ...
                env_folder4 = env_chunk[3]["folder"] 
                env_name4 = env_chunk[3]["name"]
                
                env_src1 = env_folder1/(env_name1 + ".png") # Sy: It is path of the Envmap file.
                env_src2 = env_folder2/(env_name2 + ".png")
                env_src3 = env_folder3/(env_name3 + ".png")
                env_src4 = env_folder4/(env_name4 + ".png")
                ...
                envmap_img4 = self.render_preprocess(env_src4, env_name4)
                
                dir1, dir2, dir3, dir4 = self.render_env(folder, pbrmap, envmap_img1, env_name1, devices[0]), \
                                    self.render_env(folder, pbrmap, envmap_img2, env_name2, devices[1]), \
                                    self.render_env(folder, pbrmap, envmap_img3, env_name3, devices[2]), \
                                    self.render_env(folder, pbrmap, envmap_img4, env_name4, devices[3]) # Sy: 
                common = dir1.parent
                d1, d2, d3, d4 = dir1.stem, dir2.stem, dir3.stem, dir4.stem

                shutil.copyfile(env_src1, dir1/(env_name1 + ".png")) # Sy: Copy the envmap to render folder. 
                shutil.copyfile(env_src2, dir2/(env_name2 + ".png"))
                shutil.copyfile(env_src3, dir3/(env_name3 + ".png"))
                shutil.copyfile(env_src4, dir4/(env_name4 + ".png"))

                
                print(f"{common}, {d1}, {d2}, {d3}, {d4}")
            ...
            #     e_src =  env_folder / (env_name + ".png")
            #     envmap_img = self.render_preprocess(e_src, env_name) # Sy: This tensor range is [0,1]. It uses for input of Mitsuba3 render.
                ...
                #         render_tensor = render_fn.forward_multiview(envmap_img.to(self.device), pbrmap.to(self.device)) # Sy: render_tensor is range [0,1]. It should be changed to [-1,1].
                #         render_image = TF.to_pil_image(render_tensor)
                #         print(save_dir)
                #         render_image.save(save_dir/"render.png")
...
if __name__ == "__main__":
    ...
    # root = Path('/home/sogang/mnt/db_2/oh/EnvMat/data/test')
    root = Path('/home/sogang/mnt/db_2/oh/EnvMat/data/train')
